...
variableBufferAnalysis <- function() {
  ...
  species_list <- read_csv(
    "data/processed_occurrence/model_data20251216.csv"
  ) |>
    dplyr::select("taxon") |>
    dplyr::pull() |>
    unique()
  ...
  for (species in species_list) {
    ...
    if (!overwrite && file.exists(results_path) && file.exists(stats_path)) {
      ...
      all_stats[[species]] <- readr::read_csv(
        stats_path,
        show_col_types = FALSE
      )
      next
    ...
    # Load species inputs
    sp1 <- sf::st_read(
      file.path(p1, "occurances/spatialData.gpkg"),
      quiet = TRUE
    )
