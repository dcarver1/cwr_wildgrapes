...
run_variable_buffer_analysis <- function(species, 
  ...
  for(proj in projections) {
    ...
    if(proj == "EqualArea") {
      ...
      protectedAreas_proj <- terra::project(protectedAreas, ea_proj, method = "near")
    ...
    fcsin <- fcs_insitu(srsin = srsin, grsin = grsin, ersin = ersin)
    ...
    for(b_km in buffer_sizes_km) {
      ...
      grsex <- grs_exsitu(speciesData = counts, ga50 = g_bufferCrop, thres = thres_proj)
      ersex <- ers_exsitu(speciesData = counts, thres = thres_proj, natArea = natArea_proj, ga50 = g_bufferCrop)
