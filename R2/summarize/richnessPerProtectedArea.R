...
protectedAreaRichness <- function(speciesRichness, pathToProGPKG, countries) {
  ...
  for (i in seq_along(files)) {
    ...
    s1 <- st_read(files[i]) |>
      dplyr::filter(MARINE %in% c("0", "1"), ISO3 %in% id1)
    # run the extraction
...
wdpaVect <- function(species, runVersion) {
  ...
  # grab all the spatial files of interest
  files <- list.files(
    path = "data/geospatial_datasets/protectedLands/WDPA_Mar2023_Public_shp",
    pattern = ".gpkg",
    full.names = TRUE,
    recursive = TRUE
  )
