...
compileConservationData <- function(directory, runVersion, genus) {
  ...
  runInsitu <- insitu[grepl(pattern = runVersion, x = insitu)] |>
    map(.f = read_csv) |>
    bind_rows() %>%
    select("ID", "SRSin" = SRS, "GRSin" = GRS, "ERSin" = ERS)
  runExsitu <- exsitu[grepl(pattern = runVersion, x = exsitu)] |>
    map(.f = read_csv) |>
    bind_rows() %>%
    select("ID", "SRSex" = SRS, "GRSex" = GRS, "ERSex" = ERS)
  ...
  fig <- plot_ly(
    ...
    layout(
      ...
      legend = list(
        title = list(text = '<b> Conservation Status </b>'),
        orientation = 'h'
      ),
      annotation = list(
    ...
    add_annotations(
      x = c(0.08, 0.37, 0.62, 0.88),
      y = 0.96,
      yshift = 20,
      text = c(
        "<b>urgent <br> priority</b>",
        "<b>high <br> priority</b>",
        "<b>medium <br> priority</b>",
        "<b>low <br> priority</b>"
      ),
      xref = "paper",
      yref = "paper",
