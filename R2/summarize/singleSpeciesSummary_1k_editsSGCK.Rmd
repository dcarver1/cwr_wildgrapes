...
if(data$NoModel == FALSE){
  ...
  if (species %in% c("Vitis riparia", "Vitis rotundifolia", "Vitis tiliifolia",
                     "Vitis_riparia", "Vitis_rotundifolia", "Vitis_tiliifolia")) {
    resampleFactor <- 5
    
    threshold <- terra::aggregate(threshold, fact = resampleFactor, fun = "max", na.rm = TRUE)
      
    if (!is.null(data$protectedArea)) {
      data$protectedArea <- terra::aggregate(data$protectedArea, fact = resampleFactor, fun = "max", na.rm = TRUE)
    }
    
    if (!is.null(data$projectedResults$mean)) {
      data$projectedResults$mean <- terra::aggregate(data$projectedResults$mean, fact = resampleFactor, fun = "mean", na.rm = TRUE)
    }
    if (!is.null(data$projectedResults$median)) {
      data$projectedResults$median <- terra::aggregate(data$projectedResults$median, fact = resampleFactor, fun = "mean", na.rm = TRUE)
    }
    if (!is.null(data$projectedResults$stdev)) {
      data$projectedResults$stdev <- terra::aggregate(data$projectedResults$stdev, fact = resampleFactor, fun = "mean", na.rm = TRUE)
    }
    
    if (!is.null(data$g_bufferCrop) && inherits(data$g_bufferCrop, "SpatRaster")) {
      data$g_bufferCrop <- terra::aggregate(data$g_bufferCrop, fact = resampleFactor, fun = "max", na.rm = TRUE)
    }
  }
  # -------------------------------------------------------
...
occData$popup <- paste0(
  "<b>", occData$taxon, "</b><br/>",
  occData$type, " (", occData$databaseSource, ")"
)
...
m2 <- m1 |> 
  # threshold 
  addRasterImage(x = threshold_trim,
                 colors = thresColor, 
    ...
    leaflet::addLegend(position = "bottomright",
            ...
            group = "Potential distribution model")|>
  #mean 
  addRasterImage(x = meanDist_trim, 
                 colors = meanPal,
    ...
    leaflet::addLegend(position = "bottomright",
            ...
            group = "Mean Distribution Model")|>
  #median 
  addRasterImage(x = medianDist_trim, 
                 colors = medianPal,
    ...
    leaflet::addLegend(position = "bottomright",
            ...
            group = "Median Distribution Model")|>
    #stdev 
  addRasterImage(x = stdevDist_trim, 
                 colors = stdevPal,
