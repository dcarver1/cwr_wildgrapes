...
occData$popup <- paste0("<b>",
                        occData$taxon,
                        "</b><br/>",
                        occData$type,
                        " (",
                        occData$databaseSource,
                        ")")
...
if (data$NoModel == FALSE) {
  ...
  meanDist_trim <- raster::trim(meanDist)
  medianDist_trim <- raster::trim(medianDist)
  stdevDist_trim <- raster::trim(stdevDist)
...
<br>
```{r, echo=FALSE, eval=TRUE}
auc1 <- data$aucMetrics |>
dplyr::mutate(across(where(is.numeric), sigfig)) |>
  dplyr::select(-Reason)

names(auc1) <- c("Mean AUC", "SDAUC", "AUC15", "Passed Validity Tests")
  ...
  ex3 <- exE |>
    addLayersControl(
      ...
      options = list(collapsed = FALSE,
                     autoZIndex = FALSE),
      overlayGroups = c("Potential distribution model",
        "GRS ex situ",
        "ERS ex situ",
        "Missing ecoregions",
        "All ecoregions"
