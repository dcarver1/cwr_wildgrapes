...
srs1 <- terra::rasterize(vect(occData), threshold, fun = "sum") |>
  mask(mask1)
...
# determine this ecoregions with model areas 
n1 <-data$naturalArea %>% 
    dplyr::select(ECO_ID_U)%>% 
    vect()
## zonal Stats to determine overlap 
v1 <- terra::zonal(x = thres,z = n1,fun="sum",na.rm=TRUE)# , exact = TRUE) 
# the extract parament could be a good way of getting the total areas over overlap 
# between the features but it takes a while to run. so either add it to the modeling 
# process or leave it out. 
...
## ID ecoregions in threshold model with no 
ers2 <- ers1 |>
  dplyr::filter(ECO_ID_U %in% ecosInThres)|>
  dplyr::filter(layer == 0)
if(nrow(ers2)>0){
  ersinMap <- n1 |>
    filter(ECO_ID_U %in% ers2$ECO_ID_U)
}else{
  ersinMap <- "No values"
}
...
proColor <- c("#7570b3")
# srsin palete 
srsPal <- colorNumeric(palette = distColor,domain = values(srs1), na.color = "transparent")
...
### Table of Unconserved Ecoregions 
...
in3 <- in2 |>
  ...
  addLegend(colors = exPal,
            group = "GRSin",
            labels = c("Native Area", "Predicted Habit", "Protected Area"))
## ERS in map 
...
in3 <- in3|> 
  addLayersControl(
    ...
    overlayGroups = c(
      ...
      "ERSin",
      "Protected Areas"))|>
  hideGroup(group = c("Protected Areas"))
