...
getPalette = colorRampPalette(RColorBrewer::brewer.pal(length(ers2$ECO_ID_U), "Set1"))
# define a palette option so that we can set a transparent NA value
ecoINColor <- colorNumeric(
  palette = getPalette,
...
if(class(ersinMap) != "character"){
  ...
  addRasterImage(
    x = raster(ersinMap),
    colors = getPalette(length(ers2$ECO_NAME)),
    group = "ERS in situ",
    project = FALSE
  )|>
    addLegend(colors = getPalette(length(ers2$ECO_NAME)),
              group = "ERS in situ",
