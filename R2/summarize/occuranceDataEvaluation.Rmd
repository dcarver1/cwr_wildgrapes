...
natureSeverData <- read_csv("~/Documents/cwr_wildgrapes/data/processed_occurance/natureServe.csv")
# valid lat long datasets 
observationData <- read_csv("~/Documents/cwr_wildgrapes/data/processed_occurance/testFor0905CountyMaps.csv")
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
  ...
  plantsData <- usdaPlants %>% filter(plant_symbol %in% speciesSym$plant_symbol)%>%
    mutate(usdaPlants = str_sub(geoid, start = 3, end = 7))%>%
    select(usdaPlants)
  ...
  countyGathered <- countySHP %>%
    ...
    mutate(
      plants = case_when(
        CODE_LOCAL %in% plantsData$usdaPlants ~ "USDA_Plants"),
      bonap = case_when(
...
- Germplasm sample has been collected from the county from after 1970 
- Two herbarium samples have been collected form the county from after 1970
...
- A single herbarium record was noted afer 1970
