...
definePaths <- function(dir1, j, runVersion) {
  ...
  # run level folder -------
  path <- c(path, runLevel = paste0(path$rootDir, "/", runVersion))
  ...
  path <- c(
    ...
    #all gap metrics combined
    allGapMetrics = paste0(path$gap_analysis, "/all_gapMetrics.csv"),
    # summary html path
    summaryHTMLPath = paste0(
